...
try {
  sharp = require('sharp');
  console.log('✅ Sharp available - will optimize images');
} catch {
  console.log('⚠️ Sharp not available - will copy images without optimization');
...
async function main() {
  ...
  console.log(`📁 Found ${webpFiles.length} splash images\n`);
  ...
  console.log(`👤 ${sortedChampions.length} champions\n`);
  ...
  for (const file of webpFiles) {
    ...
    if (processed % 100 === 0 || processed === webpFiles.length) {
      process.stdout.write(`\r⏳ Processing: ${processed}/${webpFiles.length}`);
    }
  }
  ...
  console.log(`📊 Size: ${originalMB}MB → ${optimizedMB}MB (${savedPercent}% saved)\n`);
...
`;
  ...
  console.log('✅ Done!\n');
  console.log(`   📁 ${webpFiles.length} images in assets/champions/splash/`);
  console.log(`   📄 index.ts generated with ${sortedChampions.length} champions\n`);
}
