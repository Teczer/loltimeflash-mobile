...
interface IClientToServerEvents {
  ...
  'game:flash:cancel': (data: { role: TRole }) => void
  'game:toggle:item': (data: { role: TRole; item: 'lucidityBoots' }) => void
  'game:flash:adjust': (data: {
    role: TRole
...
export const useSocket = (
  ...
  const toggleItem = useCallback((role: TRole, item: 'lucidityBoots'): void => {
    if (socketRef.current) {
  ...
  const adjustTimer = useCallback(
    (role: TRole, adjustmentSeconds: number): void => {
      if (socketRef.current) {
  ...
  const updateChampionData = useCallback(
    (
      ...
      if (socketRef.current) {
        socketRef.current.emit('game:champion:update', {
          roleMapping,
          gameInfo,
        })
      }
